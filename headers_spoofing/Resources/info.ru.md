...
## Влияние
Подмена IP-адреса может существенно повлиять на веб-приложения, регистрирующие действия пользователей. Подмена приводит к неточностям в исторических данных и отравлению журналов. Злоумышленники, использующие подмену IP-адреса, могут манипулировать журналами, что затрудняет точное отслеживание законных и злонамеренных действий.

## Как предотвратить
Проверка и очистка входных данных:
- внедрите строгую проверку входных данных, включая заголовки. 
- поствьте ограничения доступа для ip адресов
- логируйте все попытки доступа с непроверенных алресов, чтобы обнаруживать уязвимости и реагировать на них
- простатривайте эти логи регулярно
